...
public class Upload
{
    ...
    /// <param name="filename">The filename for the uploaded image (default: <c>image.jpg</c>).</param>
    /// <param name="mimeType">The MIME type of the image (default: <c>image/jpeg</c>).</param>
    /// <param name="source">An optional source identifier. When omitted, the server defaults to <c>camera</c>.</param>
    /// <param name="password">An optional session password.</param>
    /// <returns>The upload result containing the image ID and delivery information.</returns>
        ...
        string? password = null)
    {
        ...
        // The server's "default" encryption mode reads a multipart file whose
        // bytes are the JSON-serialized encrypted envelope (camelCase keys),
        // then RSA-OAEP-SHA256 + AES-256-GCM decrypts it. Mirror that exactly:
        // serialize the envelope with camelCase keys regardless of the SDK's
        // default snake_case JSON policy.
        ...
        var envelopeBytes = System.Text.Encoding.UTF8.GetBytes(envelope);

        using var content = new MultipartFormDataContent();
        var fileContent = new ByteArrayContent(envelopeBytes);
        fileContent.Headers.ContentType = new MediaTypeHeaderValue("application/octet-stream");
        content.Add(fileContent, "file", "image.jpg.enc");

        // Only send a source part when the caller provided one — the server
        // validates it against its ImageSource enum and defaults to "camera"
        // when absent. Never send an SDK-specific default that the enum rejects.
        if (source is not null)
        {
            content.Add(new StringContent(source), "source");
        }
